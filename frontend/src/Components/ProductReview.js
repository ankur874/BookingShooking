...
const ProductReviewCard = (props) => {
    ...
    const getName = async () => {
        const data = await axios.get(`/api/users/${props.review.userid}`)
        setName(data.data.name);
    }
    console.log(props.review)
    return (
        <div class="flex items-start p-5" style={{ border: '1px solid #F3F4F6' }} >
            ...
            <div class="ml-6">
                ...
                <div class="mt-3 w-60 text-justify">
                    <span class="font-bold">{props.review.title}</span>
                    <p class="mt-1">{props.review.review} </p>
                </div>
